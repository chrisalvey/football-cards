// Version: 1.0.6 - Complete Firebase Backend (GitHub Pages Compatible)
...
GameState.prototype.initializeSharedRoom = function() {
    var self = this;
    console.log('🔗 Connecting to shared room...');
...
GameState.prototype.startNewHand = function() {
    ...
    if (typeof createDeck !== 'function') {
        console.error('❌ createDeck function not found! Make sure cards.js is loaded.');
    ...
    console.log('✅ Hand started, dealt', this.playerHand.length, 'cards');
};
...
GameState.prototype.showGameScreen = function() {
    ...
    if (nameEl) {
        ...
        console.log('✅ Set player name in UI:', this.currentPlayer);
    } else {
...
GameState.prototype.setupEventListeners = function() {
    ...
    if (nameInput) {
        nameInput.onkeypress = function(e) {
            if (e.key === 'Enter') {
                console.log('⌨️ Enter key pressed in name input');
                self.joinGame();
            }
        };
